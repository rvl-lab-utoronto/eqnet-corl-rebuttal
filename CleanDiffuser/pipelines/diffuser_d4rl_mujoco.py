...
from cleandiffuser.nn_diffusion import JannerUNet1d,DiT1d,EqNet1d
...
def pipeline(args):
    ...
    # allows for selecting from different architecture classes 
    if args.planner_net == "transformer":
        ...
        nn_diffusion = JannerUNet1d(
            ...
            attention=False, 
            kernel_size=5)
    elif args.planner_net == 'eqnet':
        nn_diffusion = EqNet1d(
            obs_dim + act_dim,
            kernel_expansion_rate=args.eqnet_ker,
            model_dim = args.planner_emb_dim, # note - model dim doesn't do much for EqNet compared to channel dim but these need to be aligned for some reason
            emb_dim = args.planner_emb_dim,
            channel_dim = 256, 
            n_layers = 15)
